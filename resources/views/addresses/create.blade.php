...
@section('title', 'Créer une nouvelle adresse')
...
<div class="dashboard-content">
    <div class="row mb-4">
        <div class="col-12">
            ...
            <p class="text-muted">Ajoutez une nouvelle adresse pour vos collectes et livraisons</p>
        </div>
    </div>
    ...
    <div class="card custom-card">
        <div class="card-body">
            <form method="POST" action="{{ route('addresses.store') }}" class="needs-validation" id="addressForm">
                @csrf
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <select id="name" class="form-select @error('name') is-invalid @enderror" name="name" required>
                                <option value="Domicile" {{ old('name') == 'Domicile' ? 'selected' : '' }}>Domicile</option>
                                <option value="Bureau" {{ old('name') == 'Bureau' ? 'selected' : '' }}>Bureau</option>
                                <option value="Autre" {{ old('name') == 'Autre' ? 'selected' : '' }}>Autre</option>
                            </select>
                            @error('name')
                        ...
                        <div class="mb-3">
                            ...
                            @error('city_id')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                            <div class="form-text delivery-fee-info" style="display: none;">
                                Frais de livraison: <span class="fee-amount"></span>
                            </div>
                        </div>
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <select id="type" class="form-select @error('type') is-invalid @enderror" name="type" required>
                                <option value="both" {{ old('type') == 'both' ? 'selected' : '' }}>Collecte et livraison</option>
                                <option value="pickup" {{ old('type') == 'pickup' ? 'selected' : '' }}>Collecte uniquement</option>
                                <option value="delivery" {{ old('type') == 'delivery' ? 'selected' : '' }}>Livraison uniquement</option>
                            </select>
                            @error('type')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
...
function loadDistricts(cityId) {
    ...
    if (districtSelect.options.length <= 1) {
        ...
        const oldValue = "{{ old('district') }}";
        if (oldValue) {
            const option = districtSelect.querySelector(`option[value="${oldValue}"]`);
            if (option) {
                option.selected = true;
                updateFeeInfo(oldValue);
            }
        }
    }
}
...
document.getElementById('addressForm').addEventListener('submit', function(e) {
    ...
    .catch(error => {
        ...
        alert('Une erreur est survenue lors de la création de l\'adresse.');
    });
});
</script>
@endpush
