                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('collection_address_id') is-invalid @enderror" id="collection_address_id" name="collection_address_id" required>
                            ...
                            @forelse($addresses as $address)
                                <option value="{{ $address->id }}" data-city-id="{{ $address->city_id }}" @if(old('collection_address_id') == $address->id) selected @endif>
                                    {{ $address->name }} - {{ $address->address }}, {{ $address->district }}
                                </option>
                            @empty
                                <option value="" disabled>Aucune adresse disponible</option>
                            @endforelse
                        </select>
                ...
                <div class="form-step" id="step-2" data-form-step="2">
                    ...
                    <div id="delivery-address-fields" class="mb-3" @if(old('same_address_for_delivery') || (isset($tempCart['same_address_for_delivery']) && $tempCart['same_address_for_delivery'])) style="display: none;" @endif>
                        ...
                        <select class="form-select @error('delivery_address_id') is-invalid @enderror" id="delivery_address_id" name="delivery_address_id" @if(!old('same_address_for_delivery') && !(isset($tempCart['same_address_for_delivery']) && $tempCart['same_address_for_delivery'])) required @endif>
                            <option value="">Sélectionner une adresse</option>
                            @forelse($addresses as $address)
                                <option value="{{ $address->id }}" data-city-id="{{ $address->city_id }}" @if(old('delivery_address_id') == $address->id || (isset($tempCart['delivery_address_id']) && $tempCart['delivery_address_id'] == $address->id)) selected @endif>
                                    {{ $address->name }} - {{ $address->address }}, {{ $address->district }}
                                </option>
                            @empty
                                <option value="" disabled>Aucune adresse disponible</option>
...
document.addEventListener('DOMContentLoaded', function() {
    // Vérification de la disponibilité des adresses
    const collectionAddress = document.getElementById('collection_address_id');
    const deliveryAddress = document.getElementById('delivery_address_id');
    
    if (collectionAddress && collectionAddress.options.length <= 1) {
        // Aucune adresse disponible
        const alertDiv = document.createElement('div');
        alertDiv.className = 'alert alert-warning mt-3';
        alertDiv.innerHTML = `
            <i class="bi bi-exclamation-triangle me-2"></i>
            Vous devez d'abord ajouter une adresse avant de pouvoir créer une commande.
            <div class="mt-2">
                <a href="{{ route('addresses.create', ['redirect_to_order_creation' => true]) }}" class="btn btn-sm btn-primary">
                    <i class="bi bi-plus-circle"></i> Ajouter une adresse
                </a>
            </div>
        `;
        collectionAddress.parentNode.insertBefore(alertDiv, collectionAddress.nextSibling);
    }
    ...
    function calculateDeliveryFee() {
        ...
        if (!collectionAddress || !deliveryAddress) return 2000; // Frais par défaut
        
        let cityId = null;
        if (sameAddress) {
            const selectedOption = collectionAddress.options[collectionAddress.selectedIndex];
            cityId = selectedOption ? selectedOption.dataset.cityId : null;
        } else {
            const selectedOption = deliveryAddress.options[deliveryAddress.selectedIndex];
            cityId = selectedOption ? selectedOption.dataset.cityId : null;
        }
        
        if (!cityId) return 2000; // Frais par défaut
        
        const deliveryFee = window.deliveryFeesData.find(fee => fee.city_id == cityId);
        return deliveryFee ? deliveryFee.amount : 2000;
    }
    ...
    function updateSummary() {
        ...
        document.querySelectorAll('.item-quantity').forEach(input => {
            ...
            const itemTotal = quantity * weight * window.laundryPricePerKg;
            articlesSubtotal += itemTotal;
        ...
        if (voucherStatus && voucherStatus.dataset.valid === 'true') {
            totalDiscount += deliveryFee; // Livraison gratuite
        }
        
        if (couponStatus && couponStatus.dataset.valid === 'true') {
            const coupon = JSON.parse(couponStatus.dataset.coupon || '{}');
            if (coupon.discount_type === 'percentage') {
                totalDiscount += (articlesSubtotal * coupon.discount_value / 100);
            } else {
        ...
        const total = articlesSubtotal + deliveryFee - totalDiscount;

        // Mise à jour des affichages
        document.getElementById('totalWeightDisplay').textContent = totalWeight.toFixed(2);
        document.getElementById('articlesPrice').textContent = articlesSubtotal.toLocaleString();
        document.getElementById('deliveryFeeDisplay').textContent = deliveryFee.toLocaleString();
        document.getElementById('estimatedPriceDisplay').textContent = total.toLocaleString();

        // Mise à jour du récapitulatif
        document.getElementById('summary-weight').textContent = totalWeight.toFixed(2);
        document.getElementById('summary-articles-price').textContent = articlesSubtotal.toLocaleString();
        document.getElementById('summary-delivery-fee').textContent = deliveryFee.toLocaleString();
        document.getElementById('summary-total').textContent = total.toLocaleString();

        // Mise à jour de la section paiement
        document.getElementById('payment-articles-price').textContent = articlesSubtotal.toLocaleString();
        document.getElementById('payment-delivery-fee').textContent = deliveryFee.toLocaleString();
        document.getElementById('payment-total').textContent = total.toLocaleString();

        // Gestion des badges de livraison gratuite
        const showFreeDelivery = voucherStatus && voucherStatus.dataset.valid === 'true';
        document.getElementById('freeDeliveryBadge').style.display = showFreeDelivery ? 'inline' : 'none';
        document.getElementById('summary-free-delivery-badge').style.display = showFreeDelivery ? 'inline' : 'none';
        document.getElementById('payment-free-delivery-badge').style.display = showFreeDelivery ? 'inline' : 'none';
        ...
        if (totalDiscount > 0) {
            ...
            paymentDiscountRow.style.display = '';
            document.getElementById('summary-discount-value').textContent = totalDiscount.toLocaleString();
            document.getElementById('payment-discount-value').textContent = totalDiscount.toLocaleString();
        } else {
        ...
        console.log('Mise à jour du récapitulatif:', {
            ...
            totalDiscount,
            total,
            couponStatus: couponStatus ? couponStatus.dataset.valid : 'non trouvé',
            voucherStatus: voucherStatus ? voucherStatus.dataset.valid : 'non trouvé'
        });
    }
    ...
    if (applyCouponBtn && couponInput) {
        applyCouponBtn.addEventListener('click', function() {
            ...
            if (!code) {
                couponMessage.innerHTML = '<div class="alert alert-warning">Veuillez entrer un code promo</div>';
                return;
            }

            // Appel AJAX pour valider le code promo
            ...
            .then(data => {
                if (data.valid) {
                    ...
                    couponStatus.dataset.coupon = JSON.stringify(data.coupon);
                    couponMessage.innerHTML = '<div class="alert alert-success">Code promo appliqué avec succès!</div>';
                    // Forcer la mise à jour du récapitulatif
                    updateSummary();
                } else {
            ...
            .catch(error => {
                ...
                couponMessage.innerHTML = '<div class="alert alert-danger">Une erreur est survenue lors de la validation du code promo</div>';
            });
        });
    }
    ...
    if (applyVoucherBtn && voucherInput) {
        applyVoucherBtn.addEventListener('click', function() {
            ...
            if (!code) {
                voucherMessage.innerHTML = '<div class="alert alert-warning">Veuillez entrer un code de bon de livraison</div>';
                return;
            }

            // Appel AJAX pour valider le bon de livraison
            ...
            .then(data => {
                if (data.valid) {
                    ...
                    voucherMessage.innerHTML = '<div class="alert alert-success">Bon de livraison appliqué avec succès!</div>';
                    updateSummary(); // Mise à jour du récapitulatif
                } else {
            ...
            .catch(error => {
                console.error('Erreur:', error);
                voucherMessage.innerHTML = '<div class="alert alert-danger">Une erreur est survenue lors de la validation du bon de livraison</div>';
            });
        });
    }

    // Gestion de l'affichage des champs de code promo et bon de livraison
    ...
    if (useCouponCheckbox && couponFields) {
    ...
    if (useVoucherCheckbox && voucherFields) {
    ...
    // Ajout des écouteurs d'événements pour les quantités et adresses
    document.querySelectorAll('.item-quantity').forEach(input => {
    ...
    document.getElementById('collection_address_id').addEventListener('change', updateSummary);
    document.getElementById('delivery_address_id').addEventListener('change', updateSummary);
    document.getElementById('same_address_for_delivery').addEventListener('change', updateSummary);
