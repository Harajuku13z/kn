...
<div class="container-bootstrap">
    <div class="header-actions">
        <h1 class="main-title">Modifier la commande #{{ $order->id }}</h1>
        <div>
            <a href="{{ route('orders-management.index') }}" class="btn btn-secondary-custom me-2">
                <i class="bi bi-arrow-left"></i> Retour à la liste
            </a>
            <a href="{{ route('orders-management.show', $order) }}" class="btn btn-info text-white">
                <i class="bi bi-eye"></i> Voir les détails
            </a>
        </div>
    </div>

    <form action="{{ route('orders-management.update', $order) }}" method="POST">
        @csrf
        @method('PUT')
                            ...
                            <div class="row g-0">
                                ...
                                <div class="col-8">
                                    <div class="card-body">
                                        ...
                                        <div class="input-group input-group-sm quantity-selector">
                                            <button class="btn btn-outline-secondary quantity-minus" type="button">-</button>
                                            <input type="number" class="form-control text-center item-quantity" value="{{ $order->items->where('item_id', 'robe')->first()->quantity ?? 0 }}" min="0" aria-label="Quantité Robe" name="items[robe][quantity]">
        ...
        <div class="card mb-4">
            <div class="card-header bg-primary text-white">
                <h5 class="mb-0"><i class="bi bi-receipt me-2"></i> Statut de la commande</h5>
            </div>
            <div class="card-body">
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-select" id="order_status" name="order_status">
                            ...
                            <option value="processing" {{ old('order_status', $order->order_status) == 'processing' ? 'selected' : '' }}>En cours de traitement</option>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-select" id="payment_status" name="payment_status">
                            ...
                            <option value="failed" {{ old('payment_status', $order->payment_status) == 'failed' ? 'selected' : '' }}>Échec</option>
                            <option value="refunded" {{ old('payment_status', $order->payment_status) == 'refunded' ? 'selected' : '' }}>Remboursée</option>
                        </select>
                    </div>
                </div>
            </div>
        ...
        <div class="d-flex justify-content-between">
            <a href="{{ route('orders-management.index') }}" class="btn btn-secondary-custom">Annuler</a>
            <button type="submit" class="btn btn-primary-custom">Mettre à jour la commande</button>
        </div>
    </form>
</div>
@endsection
