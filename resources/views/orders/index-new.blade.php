...
<div class="container-fluid dashboard-content order-list-page">
    ...
    <div class="card shadow-sm mb-4">
        <div class="card-body">
            <form method="GET" action="{{ route('orders.index') }}" id="orderFilterForm">
                <div class="row g-3 align-items-end">
                    <div class="col-md-4">
                        <label for="search" class="form-label">Rechercher (ID, Adresse...)</label>
                        <div class="input-group">
                            <span class="input-group-text"><i class="bi bi-search"></i></span>
                            <input type="text" class="form-control" id="search" name="search" value="{{ request('search') }}" placeholder="Ex: #123, Rue de la Paix...">
                        </div>
                    </div>
                    <div class="col-md-3">
                        ...
                        <select class="form-select status-select" id="status" name="status">
                            ...
                            <option value="pending" {{ request('status') == 'pending' ? 'selected' : '' }} data-color="#ffc107">En attente</option>
                            <option value="collected" {{ request('status') == 'collected' ? 'selected' : '' }} data-color="#0dcaf0">Collecté</option>
                            <option value="in_transit" {{ request('status') == 'in_transit' ? 'selected' : '' }} data-color="#fff3cd">En transit</option>
                            <option value="washing" {{ request('status') == 'washing' ? 'selected' : '' }} data-color="#e6f2ff">Lavage</option>
                            <option value="ironing" {{ request('status') == 'ironing' ? 'selected' : '' }} data-color="#e6f2ff">Repassage</option>
                            <option value="ready_for_delivery" {{ request('status') == 'ready_for_delivery' ? 'selected' : '' }} data-color="#cfe2ff">Prêt pour livraison</option>
                            <option value="delivering" {{ request('status') == 'delivering' ? 'selected' : '' }} data-color="#fff3cd">En cours de livraison</option>
                            <option value="delivered" {{ request('status') == 'delivered' ? 'selected' : '' }} data-color="#28a745">Livré</option>
                            <option value="cancelled" {{ request('status') == 'cancelled' ? 'selected' : '' }} data-color="#f8d7da">Annulé</option>
                        </select>
                    </div>
                    <div class="col-md-3">
                        ...
                        <div class="input-group">
                            ...
                            <input type="text" class="form-control" id="date_range" name="date_range" value="{{ request('date_range') }}" placeholder="Du JJ/MM/AAAA au JJ/MM/AAAA">
                        </div>
                    </div>
                    <div class="col-md-2">
                        <button type="submit" class="btn btn-primary klin-btn w-100 mb-2"><i class="bi bi-funnel-fill me-1"></i> Filtrer</button>
                        <a href="{{ route('orders.index') }}" class="btn btn-outline-secondary w-100"><i class="bi bi-x-circle me-1"></i> Réinitialiser</a>
                    </div>
                </div>
            </form>
        </div>
                ...
                <div class="card order-card shadow-sm h-100" style="border-top: 3px solid {{ $statusColor }}">
                    ...
                    <div class="card-body">
                        ...
                        <div class="order-detail mb-3">
                            <div class="detail-label"><i class="bi bi-geo-alt me-1"></i> Adresse</div>
                            <div class="detail-value">
                                @if($order->deliveryAddress)
                                    {{ Str::limit($order->deliveryAddress->address_line1, 40) }}
                                    <div class="text-muted small">{{ $order->deliveryAddress->city }}</div>
                                @else
                                    {{ Str::limit($order->delivery_address_line1 ?? 'Adresse non spécifiée', 40) }}
                                    @if($order->delivery_city)
                                        <div class="text-muted small">{{ $order->delivery_city }}</div>
                                    @endif
                                @endif
                            </div>
                        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (deleteModal) {
            ...
            const orderNumberSpan = document.getElementById('order-number-to-delete');
            const confirmBtn = document.getElementById('confirmDeleteBtn');
            
            // Quand la modale est affichée, on récupère les données
            deleteModal.addEventListener('show.bs.modal', function(event) {
                ...
                // Mettre à jour le numéro de commande dans la modale
        ...
        // Ajout de colorisation au changement de statut dans le dropdown
        ...
        if (statusSelect) {
            ...
            // Appliquer la couleur au chargement si une option est déjà sélectionnée
